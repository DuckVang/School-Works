...
rows.forEach(row => {
    ...
    // let compartments = [row.slice(0, row.length / 2), row.slice(row.length / 2)]


    // for (let index = 0; index < compartments[0].length; index++) {
    //     if (compartments[1].includes(compartments[0][index])) { copies.push(compartments[0][index]); break }
    ...
    if (group.length == 3) {
        ...
        for (let index = 0; index < group[0].length; index++) {
            const element = group[0][index];

            if (group[1].includes(element) && group[2].includes(element)) {
                copies.push(element)
                group = []
                break
            }
